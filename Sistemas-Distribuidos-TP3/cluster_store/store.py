...
def write_request():
    ...
    while not success:
        ...
        print(f"[Rota write_request] | [{node_name}] | Enviando pedido para {primary_to_contact}\n", flush=True)
        try:
...
def write_primary():
    ...
    while i < len(peer_list):
        ...
        # Caso não seja possível comunicar com o nó, convoca a eleição de um novo nó
            ...
            dead_peer_protocol(operation)
            print(f"[Rota write_primary] | ⚙️ Operação de eleição terminada. Lista de peers atual --> {peer_list}", flush=True)
...
def elect_new_primary(operation):
    ...
    print(f"\n[Rota elect_new_primary] | [{node_name}] | 👑 Eleição para a operação [{operation}]!", flush=True)
...
def dead_peer_protocol(operation):
    global peer_list, primary_nodes, is_primary
        
    # Remoção do peer morto
    if primary_nodes[operation] in peer_list:
        print(f"[dead_peer_protocol] | [{node_name}] | 🧨 Removendo peer: [{primary_nodes[operation]}] para a operação: [{operation}]")
        peer_list.remove(primary_nodes[operation])

    # Elegendo o último na hierarquia
    primary_nodes[operation] = peer_list[len(peer_list)-1]
    print(f"[dead_peer_protocol] | [{node_name}] | 🟠 Novo nó primario para essa operação: {primary_nodes[operation]}")

    # Se o nó elegido é ele próprio, ativa a flag
    if node_name == primary_nodes[operation]:
        print(f"[dead_peer_protocol] | [{node_name}] | 🔑 Sou o novo primário dessa operação")
        is_primary[operation] = True
    ...
    for peer in peer_list:
        ...
        try:
            print(f"[Rota write_primary: Exception] | [{node_name}] | ⚠️ Notificando peer {peer}", flush=True)
            requests.get(f"http://{peer}:8080/elect_new_primary/{operation}", timeout=1)
        # Caso não consiga, imprime erro
        except requests.exceptions.RequestException as e:
            print(f"[Rota write_primary: Rerfesh Exception Exception] | {node_name} | ❌ Erro {e} na conexão com o servidor {peer}", flush=True)
